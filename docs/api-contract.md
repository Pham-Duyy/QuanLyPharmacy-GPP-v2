...
3. **Dựng khung dự án và CSDL local**, rồi mới viết endpoint theo thứ tự: ~~cửa hàng, người dùng, phân quyền~~ → ~~danh mục, đơn vị, giá~~ → ~~phiếu nhập~~ → ~~bán hàng~~ → ~~trả hàng, hủy hóa đơn~~ → ~~thu hồi~~. Toàn bộ mục 3 đã xong. Đơn thuốc (§12): API và màn hình quản lý (tạo, sửa, nộp, xác nhận, từ chối) đã xong, đã kiểm chứng bán thuốc kê đơn theo đơn thật hoạt động đúng đầu cuối. Chưa làm tải ảnh đơn (POST /prescriptions/{id}/images) — cần hạ tầng lưu trữ riêng, để sau. Bước tiếp theo: gắn chọn đơn thuốc vào màn hình bán hàng, để không phải gọi API tay khi bán thuốc kê đơn.
