...
3. **Dựng khung dự án và CSDL local**, rồi mới viết endpoint theo thứ tự: ~~cửa hàng, người dùng, phân quyền~~ → ~~danh mục, đơn vị, giá~~ → ~~phiếu nhập~~ → ~~bán hàng~~ → ~~trả hàng, hủy hóa đơn~~ → ~~thu hồi~~. Toàn bộ mục 3 đã xong. Đơn thuốc (§12) đã xong trọn vẹn: API, màn hình quản lý, và đã gắn vào màn hình bán hàng — chọn đơn thuốc VERIFIED hoặc PARTIALLY_DISPENSED ngay trong giỏ hàng, tự khớp dòng đơn với dòng giỏ hàng khi chỉ có một khả năng khớp. Đã kiểm chứng bán tiếp trên đơn đã bán một phần hoạt động đúng đầu cuối. Chưa làm tải ảnh đơn (POST /prescriptions/{id}/images) — cần hạ tầng lưu trữ riêng, để sau.
