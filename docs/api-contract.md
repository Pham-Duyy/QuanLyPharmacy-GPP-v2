...
3. **Dựng khung dự án và CSDL local**, rồi mới viết endpoint theo thứ tự: ~~cửa hàng, người dùng, phân quyền~~ → ~~danh mục, đơn vị, giá~~ → ~~phiếu nhập~~ → ~~bán hàng~~ → ~~trả hàng, hủy hóa đơn~~ → ~~thu hồi~~. Toàn bộ mục 3 đã xong. Đơn thuốc (§12) đã xong phần API (tạo, sửa, nộp, xác nhận, từ chối); chưa làm tải ảnh đơn (POST /prescriptions/{id}/images) vì cần hạ tầng lưu trữ riêng (kiểm tra nội dung tệp, xóa EXIF, URL có chữ ký) — để sau. Còn thiếu màn hình quản lý đơn thuốc trên giao diện.
